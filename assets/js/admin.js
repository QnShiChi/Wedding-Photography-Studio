...
function openDatabase() {
    return new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = function(event) {
        ...
        if (!db.objectStoreNames.contains('albums')) {
          const objectStore = db.createObjectStore('albums', { keyPath: 'albumName' });
          objectStore.createIndex('coverImage', 'coverImage', { unique: false });
        }
      };
  ...
  // Lưu ảnh đại diện vào IndexedDB
  function updateCoverImage(albumName) {
    const coverImageUpload = document.getElementById(`coverImageUpload${albumName}`).files[0];
    if (coverImageUpload) {
      const reader = new FileReader();
      reader.onload = function(event) {
        const coverImageData = event.target.result;
        ...
        openDatabase().then((db) => {
          ...
          objectStore.get(albumName).onsuccess = function(event) {
            let albumData = event.target.result || { albumName: albumName, coverImage: '' };
            albumData.coverImage = coverImageData;
  
            objectStore.put(albumData).onsuccess = function() {
              console.log(`Ảnh đại diện cho ${albumName} đã được cập nhật.`);
              displayCoverImage(albumName);  // Cập nhật ảnh đại diện trên giao diện admin
            };
          };
        });
      };
      reader.readAsDataURL(coverImageUpload);
    } else {
